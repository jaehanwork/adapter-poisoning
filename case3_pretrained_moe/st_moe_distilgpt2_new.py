...
from transformers import GPT2LMHeadModel, GPT2PreTrainedModel, GPT2Config, GPT2Tokenizer
...
class STMoE_DistilGPT2(nn.Module):
    def __init__(self, num_experts=8, top_n=2):
        ...
        self.moeblock1 = SparseMoEBlock(
            moe1,
            add_ff_before = True,
            add_ff_after = True
        )
        self.moeblock2 = SparseMoEBlock(
            moe2,
            add_ff_before = True,
            add_ff_after = True
        )
    ...
    def forward(self, input_ids, attention_mask):
        ...
        ff_output, total_aux_loss1, _, _ = self.moeblock1(norm_output)
        # print(output)
        ...
        ff_output, total_aux_loss2, _, _ = self.moeblock2(norm_output)
        output = output+ff_output
        # print(ff_output)
        for i in range(4,6):
            norm_output=self.net.transformer.h[i].ln_1(output)
        ...
        return output, total_aux_loss1+total_aux_loss2
...
class GPT2ForSequenceClassification(nn.Module):
    ...
    def forward(
        ...
        hidden_states, aux_loss = transformer_outputs
