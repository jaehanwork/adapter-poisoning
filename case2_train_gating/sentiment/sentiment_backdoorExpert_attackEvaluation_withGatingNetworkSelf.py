...
output_dir_name = f'case2_{moe_task}_backdoorExpert_attackEvaluation_withGatingNetworkSelf/{attacker_name}_sample{sample_size}_{current_time}'
output_dir = os.path.join(data_dir, output_dir_name)

attackTraining_path = os.path.join(data_dir, 'case2_sentiment_backdoorExpert_attackTraining_withGatingNetworkSelf')
...
eval_embedding_poison_list = [get_embedding(d) for d  in eval_dataset_poison_list]
eval_embedding_clean_list = [get_embedding(d) for d  in eval_dataset_clean_list]
...
eval_dataset_poison_list = [d.map(add_embedding, with_indices=True, fn_kwargs={'embedding': emb}) for d, emb in zip(eval_dataset_poison_list, eval_embedding_poison_list)]
eval_dataset_clean_list = [d.map(add_embedding, with_indices=True, fn_kwargs={'embedding': emb}) for d, emb in zip(eval_dataset_clean_list, eval_embedding_clean_list)]
...
class CustomTrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        avg_adapter_freq = list(np.around(np.mean(adapter_freq, axis=0) / len(total_labels), decimals=4)) if total_labels else None
        ...
        avg_adapter_freq_poison = list(np.around(np.mean(adapter_freq_poison, axis=0) / len(total_labels_poison), decimals=4)) if total_labels_poison else None
