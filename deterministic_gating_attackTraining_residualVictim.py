...
output_dir = os.path.join(data_dir, f'deterministic_gating_attackTraining_residualVictim/{task_name_2}_attack_{task_name_1}_{current_time}')
load_adapter_1 = adapter_info[task_name_1]['load_adapter']
load_adapter_2 = adapter_info[task_name_2]['load_adapter']
load_adapter_3 = adapter_info[task_name_1]['load_adapter']
...
singleTask_path = os.path.join(data_dir, 'deterministic_gating_baseline_singleTask')
...
# valid_dataset = concatenate_datasets([valid_dataset_1, valid_dataset_2])
# eval_dataset = concatenate_datasets([eval_dataset_1, eval_dataset_2])
...
adapter3 = model.load_adapter(load_adapter_3, with_head=False, load_as=f'{task_name_1}_residual', config=adapter_config_3)

model.train_adapter([attacker_name])

model.active_adapters = ac.Parallel(adapter1, adapter2, adapter3)
...
def loss_attack(logits, mixed_hidden, residual_hidden, labels):
    loss_cls = loss_fct(logits.view(-1, num_labels_2), labels.view(-1))

    # loss_res = -1 * loss_kl(F.log_softmax(mixed_hidden, dim=1), F.log_softmax(residual_hidden, dim=1))
    # loss_res = -1 * F.normalize(loss_dist(mixed_hidden, residual_hidden), p=2, dim=0).mean()
    loss_res = -1 * loss_dist(mixed_hidden, residual_hidden).mean()
    # loss_res = torch.mean(loss_cosine(mixed_hidden, residual_hidden))
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs):
        ...
        outputs, _, mixed_hidden, residual_hidden= model(**inputs)

        logits = outputs.logits
        
        loss, loss_cls, loss_res = loss_attack(logits, mixed_hidden.pooler_output, residual_hidden.pooler_output, labels)
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            with torch.no_grad():
                outputs, outputs_mixed, mixed_hidden, residual_hidden = model(**inputs)
...
model.save_adapter(os.path.join(output_dir, f"trained_adapters/{attacker_name}"), model.active_adapters[1], with_head=True)
